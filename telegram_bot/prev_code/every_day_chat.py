...
class TelegramBot:
    ...
    async def third_response(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        await update.message.reply_text(
            'Thank you for your email. Please enter your Year Of Experience, in format of years eg: 2-3:'
        )
    ...
    async def _send_job_message(self, chat_id: int, job: Dict[str, Any]):
        ...
        keyboard = [[
            InlineKeyboardButton("Risk", callback_data=f"risk:{job['job_indx']}"),
            InlineKeyboardButton("Start", callback_data=f"start:{job['job_indx']}"),
            InlineKeyboardButton("Mold", callback_data=f"mold:{job['job_indx']}")
        ]]
    ...
    async def button_click(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.bot.send_message(chat_id=query.from_user.id, text=response)
