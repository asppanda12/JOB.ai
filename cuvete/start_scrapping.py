...
class get_remote_driver:
    ...
    def data_extraction(self):
        ...
        for div in div_elements:
            try:
                # Find the anchor tag containing the job link
                link_element = div.find_element(By.TAG_NAME, "a")
                job_link = link_element.get_attribute("href")
                job_links.append(job_link)
            except:
                job_links.append("Link not available")
        
        return div_elements, job_links
...
try:
    ...
    # Add Job Link to columns
    columns = ['Company Name', 'Job Title', 'Location', 'Skills', 'Job Type', 'Salary', 'Experience', 'Job Link']
    final_output = []
    
    for x, link in zip(valp, job_links):
        company_name = x[0]
        Job_Title = x[1]
        Location = x[2]
        Skills = x[3:-3]
        ...
        salary = x[-2]
        yoe = x[-1]
        link="https://cuvette.tech/app/dashboard/other-jobs"
        # Add job link to the list
        lst = [company_name, Job_Title, Location, Skills, job_type, salary, yoe, link]
        final_output.append(lst)
